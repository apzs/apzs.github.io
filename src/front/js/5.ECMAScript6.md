...
![ES6-1](./image/ES6-1.4w4pcis4fva0.jfif)

### 2. 什么是 ECMAScript

ECMAScript 是由 Ecma 国际通过ECMA-262 标准化的脚本程序设计语言。

![ES6-2](./image/ES6-2.6aq6tvkonsc0.jpg)
...
如果设置了`Symbol.toPrimitive`则对该对象进行`+`、`+=`、`==`、`!=`、`-`、`*`、`/`、`%`、`>>`、`>>>`、`<<`、`^`、`&`、`|`、`++`、`--`、`>`、`<`、`>=`、`<=`、`-=`、`*=`、`/=`、`%=`、`String(obj)`、`Number(obj)`等操作都会调用该方法，该方法可接收一个参数`hint`用于说明执行的操作。
...
再通过 `import` 导入即可。 ![es6-7](./image/es6-7.216iomdsx9mo.png)
...
输出结果： ![es11-1](./image/es11-1.2fgw7vvkb0kk.png)
...
打印结果： ![ES11-2](./image/ES11-2.17q27y7sy9mk.png)
