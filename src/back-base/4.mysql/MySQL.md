...
删除表（如果id为自增，则再次添加数据时的id为前面已经出现过的最大id+1；不管这个id是否已被删除）：

```mysql
DROP TABLE [IF EXISTS] 表名;
```

删除表，并重新创建该表（如果id为自增，则再次添加数据时的id从1开始）：
...
> 查询用户需要使用 `USE mysql;` 命令，选择使用名为`mysql`的数据库
...
- `grant create, select, update (username) on table test.user to tom@'127.0.0.1';`  => 授予tom用户在`test`的`user`表里的`创建`、`查询`、`更新username字段`的权限
...
select
	name,
	(case when age > 60 then '老年' when age > 30 then '中年' else '青年' end)
from employee;
select
	name,
...
> 由于mysql不支持全外连接，因此可以使用`union`关键字进行联合查询，即将左连接后的表和右连接后的表合并，再自动去重
>
> ```
> SELECT 字段列表 FROM 表1 LEFT [ OUTER ] JOIN 表2 ON 条件 ...
> union
...
![结构图](https://gitlab.com/apzs/image/-/raw/master/image/MySQL%E4%BD%93%E7%B3%BB%E7%BB%93%E6%9E%84_20220315034329549927.png)  
![层级描述](https://gitlab.com/apzs/image/-/raw/master/image/MySQL%E4%BD%93%E7%B3%BB%E7%BB%93%E6%9E%84%E5%B1%82%E7%BA%A7%E5%90%AB%E4%B9%89_20220315034359342837.png)  
...
InnoDB 逻辑存储结构：  
![InnoDB逻辑存储结构](https://gitlab.com/apzs/image/-/raw/master/image/%E9%80%BB%E8%BE%91%E5%AD%98%E5%82%A8%E7%BB%93%E6%9E%84_20220316030616590001.png)  
...
![二叉树](https://gitlab.com/apzs/image/-/raw/master/image/%E4%BA%8C%E5%8F%89%E6%A0%91_20220316153214227108.png)

二叉树形成链表的缺点可以用红黑树来解决：
![红黑树](https://gitlab.com/apzs/image/-/raw/master/image/%E7%BA%A2%E9%BB%91%E6%A0%91_20220316163142686602.png)
红黑树也存在大数据量情况下，层级较深，检索速度慢的问题。

为了解决上述问题，可以使用 B-Tree 结构。
B-Tree (多路平衡查找树) 以一棵最大度数（max-degree，指一个节点的子节点个数）为5（5阶）的 b-tree 为例（每个节点最多存储4个key，5个指针）

![B-Tree结构](https://gitlab.com/apzs/image/-/raw/master/image/B-Tree%E7%BB%93%E6%9E%84_20220316163813441163.png)
...
![B+Tree结构图](https://gitlab.com/apzs/image/-/raw/master/image/B+Tree%E7%BB%93%E6%9E%84%E5%9B%BE_20220316170700591277.png)
...
![MySQL B+Tree 结构图](https://gitlab.com/apzs/image/-/raw/master/image/%E7%BB%93%E6%9E%84%E5%9B%BE_20220316171730865611.png "MySQL B+Tree 结构图")
...
![Hash索引原理图](https://gitlab.com/apzs/image/-/raw/master/image/Hash%E7%B4%A2%E5%BC%95%E5%8E%9F%E7%90%86%E5%9B%BE_20220317143226150679.png "Hash索引原理图")
...
![大致原理](https://gitlab.com/apzs/image/-/raw/master/image/%E5%8E%9F%E7%90%86%E5%9B%BE_20220318194454880073.png "大致原理")
![演示图](https://gitlab.com/apzs/image/-/raw/master/image/%E6%BC%94%E7%A4%BA%E5%9B%BE_20220319215403721066.png "演示图")
...
![images](https://gitlab.com/apzs/image/-/raw/master/image/pre.png "索引流程")
...
![images](https://gitlab.com/apzs/image/-/raw/master/image/uion.png)
...
![images](https://gitlab.com/apzs/image/-/raw/master/image/keySort.png)
...
![images](https://gitlab.com/apzs/image/-/raw/master/image/artic.png )

Buffer Pool：缓冲池是主内存中的一个区域，里面可以缓存磁盘上经常操作的真实数据，在执行增删改查操作时，先操作缓冲池中的数据（若缓冲池没有数据，则从磁盘加载并缓存），然后再以一定频率刷新到磁盘，从而减少磁盘I0，加快处理速度。

![images](https://gitlab.com/apzs/image/-/raw/master/image/artic2.png )
![images](https://gitlab.com/apzs/image/-/raw/master/image/artic3.png )
![images](https://gitlab.com/apzs/image/-/raw/master/image/artic4.png )
![images](https://gitlab.com/apzs/image/-/raw/master/image/artic5.png )


磁盘架构：
![images](https://gitlab.com/apzs/image/-/raw/master/image/artic6.png )
![images](https://gitlab.com/apzs/image/-/raw/master/image/artic7.png )
![images](https://gitlab.com/apzs/image/-/raw/master/image/artic8.png )
...
![images](https://gitlab.com/apzs/image/-/raw/master/image/MVCC.png )
...
![images](https://gitlab.com/apzs/image/-/raw/master/image/MVCCList.png )

如何确定返回哪一个版本 这是由read view决定返回 undo log 中的哪一个版本。

![images](https://gitlab.com/apzs/image/-/raw/master/image/readview0.png )
...
![images](https://gitlab.com/apzs/image/-/raw/master/image/readview.png )
