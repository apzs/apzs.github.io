...
![](./image/0a86aaf523544316bdb0fbb5f4937954.png)
...
![](./image/57df9daf5d71449b85e9aa9fce7ddac6.png)

**演示：**

![](./image/5a5b0fa8d7e64c48bbe90eb11dd5207d.png)

跟xml一样，一些特殊符号不能直接打出，要用**转义字符：**

![](./image/6cabdb75b1fd4d3e8e6e74c0f44f5723.png)

#### 1.4.2 图片、音频、视频标签

![](./image/f35b9e3c461c4e58a27a2a7ce7cedb38.png)
...
![](./image/8b5dc2ff83894dbb84f90bc1b98bf2f7.png)
...
![](./image/3026296ee43346bea912935e442a4b54.png)
...
```
 ...
 ![](./image/4cea7939662247b8a84394a12aab47d7.png)
...
![](./image/4b3a92255cb74ba18468ffcfbe014ce9.png)

`form` 是表单标签，它在页面上没有任何展示的效果。需要借助于表单项标签来展示不同的效果。如下图就是不同的表单项标签展示出来的效果。  

![](./image/72ff1eaea72c41fabd759874661dd183.png)
...
![](./image/3c89f62cbdc3403ea563adcf21f1cb48.png) 如果提交方式改为post，则提交的数据会放在http请求消息体中：

![](./image/c06a3f963cac43ee81607a5b2dc9070e.png)
...
*   **`<input>`**：表单项，通过type属性控制输入形式
    ...
    ![](./image/d69118f62097424eb0a21d246e6b6d09.png)
...
![](./image/feac8a54281b4066bb18a2abf947e509.png)
...
![](./image/46e19300db2345f8bd32789fceb86672.png)
...
在浏览器的效果如下：
 ...
 ![](./image/e20ae74ef22d4f2790637848ccf55035.png)
...
![](./image/505eb409943e40468b4f67bc1d8639de.png)
...
上面代码通过浏览器打开，我们可以看到如下图弹框效果![](./image/2cb576d47347431aba75c6307873dc6f.png)
...
上面代码通过浏览器打开，我们可以在页面上看到 `document.write(内容)` 输出的内容![](./image/f585a8d67d3d4bdf8fe77787f641bbd5.png)
...
![](./image/a3a69af085b74107b4079c3aa18adddd.png)
...
运行上面代码，浏览器并没有弹框输出结果，说明这段代码是有问题的。通过 `F12` 打开开发者模式可以看到如下错误信息![](./image/412494b67faa468996d3615d6a9d9053.png)

而如果在代码块中定义两个同名的变量，IDEA 开发工具就直接报错了

![](./image/7f9d04b482594331928243293a8da741.png)
...
![](./image/c5df1489682d49fa8d1f71b8d534b8dd.png)
...
*   **null**：对象为空
    ...
    >
    > 这个官方给出了解释，下面是从官方文档截的图![](./image/3475b911a14b4f74b888739cc4aaf0b0.png)
...
![](./image/746755d4fe8f4755a521b8626458041c.png)
...
![](./image/3795a15a02964130be3e5b36299473b0.png)  
...
![](./image/74196d5125a24f61b124f68a42abeb8c.png)
...
![](./image/e51efd0d4ded489d9721aa1bdea76d58.png)
...
![](./image/18b653da61db476fb0312f250bebeccb.png)

**函数：**

String对象提供了很多函数（方法），下面给大家列举了两个方法。

![](./image/0be356c20b234601a6fe938b5ed45769.png)
...
![](./image/9e80257f7d0c4950a222f44dfedef3d5.png)
...
![](./image/b4a01ff5c3464cb8b0aac6a01d45dcaf.png)
...
![](./image/eb3adc388318401f9e06a34c2242a979.png)  
...
![](./image/768f15b805b844b79e10bdb2f6f97f46.png)

**函数**

`window` 对象提供了很多函数供我们使用，而很多都不常用；下面给大家列举了一些比较常用的函数

![](./image/06da51a214a74e23bb46eefa1ab9c526.png)  
...
![](./image/e7acd3681591453babfeeab4eef9b753.png)

而以后我们在页面删除数据时候如下图每一条数据后都有 `删除` 按钮，有可能是用户的一些误操作，所以对于删除操作需要用户进行再次确认，此时就需要用到 `confirm()` 函数。

![](./image/600aad081559471bbbb347c1f35f9157.png)
...
![](./image/a1dbaca0ebd04808be7fb0738b41a808.png)
...
![](./image/8cc1e8ced3084236b80f0c04cdcd8369.png)

当我们点击向左返回的箭头，就跳转到前一个访问的页面，这就是 `back()` 函数的作用；当我们点击向右前进的箭头，就跳转到下一个访问的页面，这就是 `forward()` 函数的作用。

#### 3.5.4 Location地址栏对象

![](./image/03a3bc0a9b00487d996c70062778b521.png)
...
![](./image/e388d64b874f480a8f7ae0ef0370ff80.png)
...
![](./image/b48b127b45294799b4dd42928679942c.png)
...
![](./image/a8821da8577f4944a643a2deb27a6008.png)
...
![](./image/dd79633c145d4c9fa2a8371ca82f9466.png)
...
![](./image/2310fa9fc18d45639815d0395160b3d1.png)
...
![](./image/decc58d382e84753823f70cedf4b236e.png)
...
![](./image/3f9be19691274c95ba3839a529489912.png)
...
![](./image/f83dec83824241c885102e118b6bdeb9.png)
...
![](./image/8e446aaf321d4fb9b8d08ebf98929d69.png)

再比如下图输入框，当我们输入了用户名 `光标离开` 输入框，就需要通过 js 代码对输入的内容进行校验，没通过校验就在输入框后提示 `用户名格式有误!`

![](./image/d806d75507a34396b06bbe104ca8e283.png)
...
![](./image/e16ba5d920f04367be2ddb127b368841.png)

**`onblur` 失去焦点事件。**

如下图，当点击了输入框后，输入框就获得了焦点；再点击页面其他位置，那输入框就失去焦点了。下图示例是将输入的文本转换为大写。

![](./image/addb4a86f5f84cd7bce4ffd575ee1370.png)
...
![](./image/da01e50f407d41ed820154d420c23a0f.png)
...
![](./image/dfeb8cfb597f4b1daa431ba3fdb12132.png)
...
![](./image/b0bd3e9f6ea94a86967d838b7ae9562e.png) 

![](./image/e6676765d6734c358be70d64c50f2282.png) 

#### 3.10.1 let & const

vscode快捷键：`！+ 回车`生成html模板

![](./image/0354bd242db54e318cbafab14d045fee.png) 
...
![](./image/76c2dc3c4db64e319f555301b51f4677.png)

![](./image/79e5a15a84164484b55584f07cb5cbb1.png) 
