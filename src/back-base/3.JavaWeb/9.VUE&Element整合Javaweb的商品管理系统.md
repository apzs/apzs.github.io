...
![](https://gitlab.com/apzs/image/-/raw/master/image/fc5c9cd248c44a8fbc2490c627efff1a.png)
...
![](https://gitlab.com/apzs/image/-/raw/master/image/40b49b737db3470cb2f348923f6fdafc.png)
...
![](https://gitlab.com/apzs/image/-/raw/master/image/6c4b047a712197ee67ea448759abaeaf.png)
...
![](https://gitlab.com/apzs/image/-/raw/master/image/26fa16ab487a44c3b718717eb7ddf397.png)
...
![](https://gitlab.com/apzs/image/-/raw/master/image/94fea37843654066a71c46f726dbcf95.png)

如上图所示点击多条数据前的复选框就意味着要删除这些数据，而点击了 `批量删除` 按钮后，需要让用户确认一下，因为有可能是用户误操作的，当用户确定后需要给后端发送请求并携带者需要删除数据的多个id值，后端程序删除数据库中的数据。具体的流程如下：

![](https://gitlab.com/apzs/image/-/raw/master/image/e3a6c3cdbfb44277b1ac329eaca0bffa.png)
...
![](https://gitlab.com/apzs/image/-/raw/master/image/aa695a228b0a4f06b0da043f71140600.png)

从上图可以看出表格复选框绑定了一个 `selection-change` 事件，该事件是当选择项发生变化时会触发。该事件绑定了 `handleSelectionChange` 函数，而该函数有一个参数 `val` ，该参数是获取选中行的数据，如下

![](https://gitlab.com/apzs/image/-/raw/master/image/b829840fc9d044e48ceacf099fe9895a.png)

而我们只需要将所有选中数据的id值提交给服务端即可，获取id的逻辑我们书写在 `批量删除` 按钮绑定的函数中。

在 `批量删除` 按钮绑定单击事件，并给绑定触发时调用的函数，如下

![](https://gitlab.com/apzs/image/-/raw/master/image/def96943b67c43d8a184185fbe244e74.png)
...
![](https://gitlab.com/apzs/image/-/raw/master/image/d0a3028d0f4e40d2bda6268817c29c8d.png)
...
![](https://gitlab.com/apzs/image/-/raw/master/image/24bc3cbdd7ea424babc1bada6215e649.png)
...
*   **后端需要响应什么数据给前端**![](https://gitlab.com/apzs/image/-/raw/master/image/3d563c04b0c847ef81b48add373c8bc8.png)
    ...
    *   **总共记录数。**在上图页面中需要展示总的记录数，所以这部分数据也需要。总的页面 elementUI 的分页组件会自动计算，我们不需要关心
    ...
    而这两部分需要封装到 **PageBean 对象**中，并将该对象转换为 json 格式的数据响应回给浏览器![](https://gitlab.com/apzs/image/-/raw/master/image/8206561dd57c4ef493bd6be7d15fc3f4.png)
...
![](https://gitlab.com/apzs/image/-/raw/master/image/046c806f5f0346cda86957a28820a0d7.png)
...
![](https://gitlab.com/apzs/image/-/raw/master/image/10671bfd4efc4e019d4130c4687f9345.png)
...
![](https://gitlab.com/apzs/image/-/raw/master/image/5f06495211c94d0d95e7ed7763cb4930.png)
...
![](https://gitlab.com/apzs/image/-/raw/master/image/0c96936fbfe84c0185dc591c74c40f1d.png)
...
![](https://gitlab.com/apzs/image/-/raw/master/image/c402b8dd73f6442c8fe895acd5732068.png)

当我们改变每页显示的条目数后，需要重新发送异步请求。而下图是分页组件代码，`@size-change` 就是每页显示的条目数发生变化时会触发的事件

![](https://gitlab.com/apzs/image/-/raw/master/image/c88aca265fa64ffab96fa91ac9026288.png)
...
![](https://gitlab.com/apzs/image/-/raw/master/image/b5c7539ff1ed4ca3b01041ad4d7feb89.png)
...
![](https://gitlab.com/apzs/image/-/raw/master/image/364ff352e8ac45b3a5b9962a15f20e0b.png)
...
![](https://gitlab.com/apzs/image/-/raw/master/image/f9e2efafcec1467e9835bf1e55cdb853.png)

整个条件分页查询流程如下

![](https://gitlab.com/apzs/image/-/raw/master/image/558ebb171f4b4c44a0f67bdad85a6cea.png)
...
1.  **查询表单绑定查询条件对象模型**
    ...
    这一步在页面上已经实现了，页面代码如下：![](https://gitlab.com/apzs/image/-/raw/master/image/277bb9f4f76a45428fe756da5584b316.png)

2.  **点击查询按钮查询数据**![](https://gitlab.com/apzs/image/-/raw/master/image/37538ced68c44235a5f11abd07bbbfa9.png)
