...
>
> ![](./image/8715aacdf9664c3f95fa7438a5e766b0.png)
>
...
> **`MVC` 思想**图解
>
> ![](./image/48b16581277c4651bd2fd4da258a3bcd.png)
>
> C 就是咱们 js 代码，M 就是数据，而 V 是页面上展示的内容，如下图是我们之前写的代码
>
> ![](./image/a9d3b3fcc19f4bbebc309bbf440b0b41.png)
>
...
![](./image/1779924b1a4e4010b2fee01d549b5281.png)
...
![](./image/9c3b742e25fb4e338489f5fdd6d845fa.png)

通过浏览器打开该页面可以看到如下页面

![](./image/c34f2bd0b2f04050b79c8153623da9ef.png)
...
![](./image/fc5c9cd248c44a8fbc2490c627efff1a.png)
...
![](./image/0ba4fca7fbac4c9c86b94c6cbebd9353.png)
...
![](./image/1ace6eddba824a34b2a402a02d23e2f6.png)

#### 1.3.2 绑定事件，v-on 指令

![](./image/87328f3e4a764cda90cf9de0abda10a3.png)
...
![](./image/c29f6b87870d47e98645b5512979b429.png)
...
![](./image/4de9745fbd784abea36c53c6bf7f182d.png)
...
![](./image/41d70243854f4580abf95d096a956755.png)
...
![](./image/81b878468faf41fe88ff61053f70cc73.png)

#### 1.3.4 列表渲染，v-for 指令

![](./image/fcf179e9badc4aab9f6ea04285f52fd4.png)
...
![](./image/7376148665f043538c1300094f0454a0.png)
...
![](./image/abe0c65d83504b07bd34360500c41ad3.png)

其他了解即可，最常**用的是created和mounted**，类似于window.onload

下图是 Vue 官网提供的从创建 Vue 到效果 Vue 对象的整个过程及各个阶段对应的钩子函数

![](./image/e70a27c4f870464fadd652cc5ce8f92a.png)
...
![](./image/bb93ca4f1ff846d4b10c56e3a2f764e8.png)
...
![](./image/40b49b737db3470cb2f348923f6fdafc.png)
...
![](./image/e39c3bdc20264a4d95038482147169ec.png)

此案例只是使用 Vue 对前端代码进行优化，后端代码无需修改。

#### 1.6.2 查询所有功能

![](./image/015c0c60297c45f4a78e3e0fd862e54f.png)
...
![](./image/a7004129975440cba1f32a785e853b6f.png)

整体流程如下

![](./image/e45789944c5b4b33afeac42922e2ccb1.png)
...
![](./image/f1c7ceb5adaa4f5e87e9f4c565617c93.png)
...
![](./image/6c4b047a712197ee67ea448759abaeaf.png)

> standalone单例是选择运行+编译：![](./image/cb5b452892a1490ba00094fde2454c61.png)
>
...
![](./image/407a68a78fd94ee39ed808d3eb5ef690.png)

**访问默认端口**[http://localhost:8080/#/](http://localhost:8080/ rel= "http://localhost:8080/#/")**。**

![](./image/7d40e948a2071e0ee2978687474627b8.png)
...
![](./image/d9ecda9add2f4ff784950873b3fa7be7.png)
...
> /config/**index.js配置端口：**
>
> ![](./image/ffb5bf1272934068ad887d4d62c21226.png)
...
**此时访问**[http://localhost:8080/#/hello](http://localhost:8080/ rel= "http://localhost:8080/#/hello")

![](./image/10549b4b55774f729aa549d3a99e3f39.png)
...
![](./image/adc6c83703344334a8e5a794aa81f0cd.png)
...
![](./image/450f71b602cc4b45b22d1a89d07d7e5d.png)
...
![](./image/ea8181cbcb394465aa9fa8ca2aea6cee.png)

接下来直接点击 `组件` ，页面如下

![](./image/2c18db2c72374e33bbe337ea1959aa0b.png)

### 2.1 快速入门，使用el-button组件

1.  将资源 `element-ui` 文件夹直接拷贝到项目的 `webapp` 下。目录结构如下

    ![](./image/8923876f471e4eadbd6ac606032920cc.png)
...
4.  官网复制Element组件代码![](./image/df4eb04157224aeab432a1b583e01b98.png)
...
![](./image/a7157881dd7945aab840d2928709f368.png)
...
![](./image/0e53f298efaa4513b44b547686c09274.png)
...
![](./image/32469cd32eee416794ab1cc6e0e01120.png)

要完成该页面，我们需要先对这个页面进行分析，看页面由哪儿几部分组成，然后到官网进行拷贝并修改。页面总共有如下组成部分

![](./image/b04ee7e86fae4907bbc25007116567cf.png)

还有一个是当我们点击 `新增` 按钮，会在页面正中间弹出一个对话框，如下

![](./image/bce722f085ac455aa596fa13efe70bd5.png)
...
![](./image/281a500577ca4aaabdd4a851da284ffb.png)
...
![](./image/f6e8a444890f4769b0717b4a9ab3a872.png)
...
1.  **修改表头和数据**
    ...
    下面是对表格代码进行分析的图解。根据下图说明修改自己的列数和列名![](./image/52f6e7e64e7640df893f00954f74ff7a.png)

    修改完页面后，还需要对绑定的模型数据进行修改，下图是对模型数据进行分析的图解![](./image/b06e2a8b79c14cfbbe10737a39d4bd65.png)

2.  **给表格添加操作列**

    从之前的表格拷贝一列出来并对其进行修改。按钮是从官网的 `Button 按钮` 组件中拷贝并修改的![](./image/704da174fe6b4913883f7c205e7d0577.png)

3.  **给表格添加复选框列和标号列**

    给表格添加复选框和标号列，效果如下![](./image/ed2b6f7030a147e39b6e49400883346b.png)

    此效果也是从 Element 官网进行拷贝，先找到对应的表格效果，然后将其对应代码拷贝到我们的代码中，如下是复选框列官网效果图和代码![](./image/aca1f5f790ef4a1f9f03157401c4ab58.png)

    这里需要注意在 `<el-table>` 标签上有一个事件 `@selection-change="handleSelectionChange"` ，这里绑定的函数也需要从官网拷贝到我们自己的页面代码中，函数代码如下：![](./image/5904ac6dbfaf479ab30fcf6601fe7bcb.png)
...
![](./image/26fa16ab487a44c3b718717eb7ddf397.png)

#### 2.3.3 完成搜索表单展示

在 Element 官网找到横排的表单效果，然后拷贝代码并进行修改

![](./image/b5ac5921856240b08d645881aaa3254a.png)

点击上面的 `显示代码` 后，就会展示出对应的代码，下面是对这部分代码进行分析的图解

inline是行内表单模式

![](./image/ea5695856c9c42ecad065494e46c4667.png)
...
![](./image/0b4f1c238a3f401698bbeea55778f61a.png)

#### 2.3.5 完成对话框展示

在 Element 官网找对话框，如下：![](./image/67f5ed13389a48988ef7f0f4304384c3.png)

下面对官网提供的代码进行分析

![](./image/149ba50c449b479daeb84869d1f04f57.png)
...
![](./image/a15111a5bd3e41f883d77ef3975c57b8.png)

点击 `显示代码` ，找到 `完整功能` 对应的代码，接下来对该代码进行分析

![](./image/ff927ed94e5f4024af4820246167c8aa.png)
...
*   `page-sizes` ： 每页显示个数选择器的选项设置。
    ...
    `:page-sizes="[100,200,300,400]"` 对应的页面效果如下：![](./image/890410ea2c8d4c69aa68378fa50d02b1.png)
...
![](./image/53ff57ff65bc4e8f95ad712744e69954.png)

下面是开发完成后的目录结构：

![](./image/5fcd526783704a88ba8435b3de2069c1.png)
...
![](./image/8cf84bf26c944879b7f3d9ea015f8d84.png)
...
*   响应的数据格式是什么样？
    ...
    ![](./image/77a8c829ad554cada2d681f0c563d1bd.png)


整体流程如下

![](./image/fd6abe95538f46f9b13862bddbf1cb98.png)
...
>
> ![](./image/0d7e57c7b5cb4390804e796d62bf75f8.png)
>
...
![](./image/3a8dc2d2cb4441e5a8bd0f24298d72ec.png)

上图是添加数据的对话框，当点击 `提交` 按钮后就需要将数据提交到后端，并将数据保存到数据库中。下图是整体的流程：

![](./image/3eef9ff94593458db6d795851fe4558c.png)

页面发送请求时，需要将输入框输入的内容提交给后端程序，而这里是以 json 格式进行传递的。而具体的数据格式如下：

![](./image/ad9b6414ae3f4f1182c3b07749bf0e25.png)
...
![](./image/70796fcb77ba446caf0bd253afdb57e7.png)
...
1.  **关闭新增对话框窗口**
    ...
    如下图所示是添加数据的对话框代码，从代码中可以看到此对话框绑定了 `dialogVisible` 数据模型，只需要将该数据模型的值设置为 false，就可以关闭新增对话框窗口了。![](./image/1b1c5e4b1e6e44a8b8c8ed102fbabf96.png)
...
3.  **弹出消息给用户提示添加成功**![](./image/085bff347d94475a8f69d9bf02c1bec6.png)
...
![](./image/d0a3028d0f4e40d2bda6268817c29c8d.png)
...
![](./image/24bc3cbdd7ea424babc1bada6215e649.png)
...
*   **后端需要响应什么数据给前端**![](./image/3d563c04b0c847ef81b48add373c8bc8.png)
    ...
    *   **总共记录数。**在上图页面中需要展示总的记录数，所以这部分数据也需要。总的页面 elementUI 的分页组件会自动计算，我们不需要关心
    ...
    而这两部分需要封装到 **PageBean 对象**中，并将该对象转换为 json 格式的数据响应回给浏览器![](./image/8206561dd57c4ef493bd6be7d15fc3f4.png)
...
![](./image/046c806f5f0346cda86957a28820a0d7.png)
...
![](./image/10671bfd4efc4e019d4130c4687f9345.png)
...
![](./image/5f06495211c94d0d95e7ed7763cb4930.png)
...
![](./image/0c96936fbfe84c0185dc591c74c40f1d.png)
...
![](./image/c402b8dd73f6442c8fe895acd5732068.png)

当我们改变每页显示的条目数后，需要重新发送异步请求。而下图是分页组件代码，`@size-change` 就是每页显示的条目数发生变化时会触发的事件

![](./image/c88aca265fa64ffab96fa91ac9026288.png)
...
![](./image/b5c7539ff1ed4ca3b01041ad4d7feb89.png)
...
![](./image/364ff352e8ac45b3a5b9962a15f20e0b.png)
...
![](./image/f9e2efafcec1467e9835bf1e55cdb853.png)

整个条件分页查询流程如下

![](./image/558ebb171f4b4c44a0f67bdad85a6cea.png)
...
1.  **查询表单绑定查询条件对象模型**
    ...
    这一步在页面上已经实现了，页面代码如下：![](./image/277bb9f4f76a45428fe756da5584b316.png)

2.  **点击查询按钮查询数据**![](./image/37538ced68c44235a5f11abd07bbbfa9.png)
