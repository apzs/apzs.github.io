...
![image-20230228163108844](./image/image-20230228163108844.png)
...
![image-20230226104953067](./image/image-20230226104953067.png)
...
![image-20230226112736264](./image/image-20230226112736264.png)
...
![image-20230227104752344](./image/image-20230227104752344.png)
...
分页查询：需求：每页2个，第二页开始：跳过前两条数据，接着值显示第3条和第4条数据
...
![index-ascending.bakedsvg](./image/index-ascending.bakedsvg.svg)
...
![index-compound-key.bakedsvg](./image/index-compound-key.bakedsvg.svg)
...
![image-20230301220208528](./image/image-20230301220208528.png)
...
![image-20230301220405211](./image/image-20230301220405211.png)
...
![image-20230301221051298](./image/image-20230301221051298.png)
...
![image-20230301221457190](./image/image-20230301221457190.png)

#### 4.4.2 涵盖的查询 

涵盖查询（Covered Queries）：当查询条件和查询的投影仅包含索引字段时，MongoDB直接从索引返回结果，而不扫描任何文档或将文档带入内存。这些覆盖的查询可以非常有效。

![image-20230228101811152](./image/image-20230228101811152.png)
...
![image-20230302095437984](./image/image-20230302095437984.png)
...
![image-20230228102219898](./image/image-20230228102219898.png)
...
![image-20230302105649383](./image/image-20230302105649383.png)
...
![image-20230302111421444](./image/image-20230302111421444.png)
...
![image-20230302112838296](./image/image-20230302112838296.png)
...
![image-20230228105215788](./image/image-20230228105215788.png)
...
![image-20230228105421331](./image/image-20230228105421331.png)
...
![image-20230228105508095](./image/image-20230228105508095.png)
...
结果，连接上之后，很多命令无法使用，，比如 `show dbs` 等，必须初始化副本集才行 准备初始化新的副本集：
...
| Parameter       | Type     | Description                                                  |
| --------------- | -------- | ------------------------------------------------------------ |
| `configuration` | document | Optional.  A document that specifies [confi](https://docs.mongodb.com/manual/reference/replica-configuration/#replica-set-configuration-document)[g](https://docs.mongodb.com/manual/reference/replica-configuration/#replica-set-configuration-document)[uration](https://docs.mongodb.com/manual/reference/replica-configuration/#replica-set-configuration-document)  for the  new  replica set. If a configuration is not specified,  MongoDB  uses a default replica set configuration. |
...
返回包含状态信息的文档。此输出使用从副本集的其他成员发送的心跳包中获得的数据反映副本集的当前状态。语法：
...
![image-20230228202843639](./image/image-20230228202843639.png)

![image-20230228202856031](./image/image-20230228202856031.png)
...
- /database如果指定username:password@，连接并验证登陆指定数据库。若不指定，默认打开test 数据库。

- ?options是连接选项。如果不使用/database，则前面需要加上/。所有连接选项都是键值对
...
| slaveOk=true\|false | true:在connect=direct模式下，驱动会连接第一台机器，即使这台  服务器不是主。在connect=replicaSet模式下，驱动会发送所有的写请求到主并且把读取操作分布在其他从服务器。false: 在 connect=direct模式下，驱动会自动找寻主服务器. 在  connect=replicaSet  模式下，驱动仅仅连接主服务器，并且所有的读写命令都连接到主服务器。 |
...
![image-20230228203653918](./image/image-20230228203653918.png)
...
![image-20230228203944095](./image/image-20230228203944095.png)
...
| Value       | Description                                                  |
| ----------- | ------------------------------------------------------------ |
| `configsvr` | Start  this instance as a [confi](https://docs.mongodb.com/manual/reference/glossary/#term-config-server)[g](https://docs.mongodb.com/manual/reference/glossary/#term-config-server)[   server](https://docs.mongodb.com/manual/reference/glossary/#term-config-server). The instance starts on port `27019`  by default. |
| `shardsvr`  | Start  this instance as a [shard](https://docs.mongodb.com/manual/reference/glossary/#term-shard)[.](https://docs.mongodb.com/manual/reference/glossary/#term-shard)  The instance starts on port 27018 by default. |
...
```bash
mongos>
sh.addShard("myshardrs02/192.168.0.2:27318,180.76.159.126:27418,180.76.159.126:2
7518")
{
...
"myshardrs02/180.76.159.126:27318,180.76.159.126:27418", "state" : 1 }
	...
	balancer:
		...
		Migration Results for the last 24 hours:
				No recent migrations
	databases:
		{ "_id" : "articledb", "primary" : "myshardrs02", "partitioned" :true, "version" : { "uuid" : UUID("788c9a3b-bb6a-4cc2-a597-974694772986"),
"lastMod" : 1 } }
		{ "_id" : "config", "primary" : "config", "partitioned" : true }
...
| `key`       | document | 用作分片键的索引规范文档。shard键决定MongoDB如何在 shard之间分发文档。除非集合为空，否则索引必须在shard  collection命令之前存在。如果集合为空，则MongoDB在对集合进行分片之前创建索引，前提是支持分片键的索引不存在。简单的说：由包含字段和该字段的索引遍历方向的文档组成。 |
...
![image-20230301094705664](./image/image-20230301094705664.png)
...
![image-20230301095319358](./image/image-20230301095319358.png)

提示：和连接单机mongod一样。

连接成功后，上方有mongos和分片集群的提示：

![image-20230301095335877](./image/image-20230301095335877.png)
...
![image-20230301102123836](./image/image-20230301102123836.png)
