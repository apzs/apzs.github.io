...
![热门推荐](https://gitlab.com/apzs/image/-/raw/master/image/hot_picture_1.png)
...
经过分析，尽管不同类型推荐的请求 url 不同，但请求参数及响应格式都具有一致性，因此可以将接口的调用进行封装。
...
电商项目较为常见商品展示，商品的类型是可复用的，封装到 `src/types/global.d.ts` 文件中：（其实就是猜你喜欢的商品类型`GuessItem`，可以复制`src/types/home.d.ts`文件的`GuessItem`，然后将名字改为`GoodsItem`）
...
由于推荐模块的商品和猜你喜欢的商品类型相同，可直接删掉`GuessItem`的内容，将`GoodsItem`类型赋值给`GuessItem`。
...
// 获取当前推荐信息
const currHot = hotMap.find((v) => v.type === query.type) || hotMap[0]
// 动态设置标题
uni.setNavigationBarTitle({ title: currHot.title })
...
![Tab 交互](https://gitlab.com/apzs/image/-/raw/master/image/hot_picture_2.png)
...
热门推荐页（热门推荐页也可以做一个骨架屏，这里就不演示了）

> 如果代码跑不起来，可以试试我写的`onScrolltolower`
>
> ```js
> // 滚动触底，加载更多商品
> const onScrolltolower = async () => {
>   // 获取当前选项
>   const currentSubTypes = subTypes.value[activeIndex.value] || subTypes.value[0] || {}
>   const goodsItems = currentSubTypes.goodsItems || {}
>   if(Number(goodsItems.page) < Number(goodsItems.pages)){
>     // 如果当前页码小于总页码，则当前页码累加
>     goodsItems.page = Number(goodsItems?.page) + 1
>   }else {
>     // 如果当前页码大于总页码，则标记数据已经加载到底部
...
>   // 追加商品数据
>   goodsItems.items?.push(...(newSubTypes.goodsItems?.items || []))
> }
> ```
